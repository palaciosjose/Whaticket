...
// Manejo del comando /start
    ...
    if ($last_sender_email_number || $last_recipient_email_number) {
        ...
        $subject_contains_Home = stripos($overview[0]->subject, "Importante: Cómo actualizar tu Hogar con Netflix") !== false;
        $body_contains_login_Home = stripos($message_body, "¿Solicitaste actualizar tu Hogar con Netflix?") !== false;


        if (
            $subject_contains_access_keyword || 
            $body_contains_access_keyword ||
            $subject_contains_login_keyword ||
            $body_contains_login_keyword ||
            $subject_contains_Home || 
            $body_contains_login_Home
        ) {
