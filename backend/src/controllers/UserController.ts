...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (!companyUser) {
    ...
    dataNowMoreTwoDays.setDate(dataNowMoreTwoDays.getDate() + 30);
    ...
    try {
      const _email = {
        ...
        text: `Hola ${name}, este es un email sobre el registro de ${companyName}!<br><br>
        A continuación los datos de su empresa:<br><br>Nombre: ${companyName}<br>Email: ${email}<br>Contraseña: ${password}<br>Fecha Vencimiento Trial: ${dateToClient(date)}`
      }
    ...
    try {
      ...
      if (whatsappCompany.whatsapps[0].status === "CONNECTED" && (phone !== undefined || !isNil(phone) || !isEmpty(phone))) {
        ...
        const body = `Hola ${name}, este es un mensaje sobre el registro de ${companyName}!\n\nA continuación los datos de su empresa:\n\nNombre: ${companyName}\nEmail: ${email}\nContraseña: ${password}\nFecha Vencimiento Trial: ${dateToClient(date)}`
...
export const forgetPassword = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    // Guardar token en el usuario (usando tokenVersion como campo temporal)
    await user.update({
      tokenVersion: resetToken
    });

    // Enviar email
    const emailData = {
      to: email,
      subject: "Recuperación de Contraseña - StreamDigi",
      html: `
        <h2>Recuperación de Contraseña</h2>
        <p>Hola ${user.name},</p>
        <p>Recibimos una solicitud para restablecer tu contraseña.</p>
        <p><strong>Tu código de verificación es:</strong></p>
        <h3 style="background: #f0f0f0; padding: 10px; font-family: monospace;">${resetToken}</h3>
        <p>Este código expira en 24 horas.</p>
        <p>Si no solicitaste este cambio, ignora este email.</p>
        <br>
        <p>Saludos,<br>Equipo StreamDigi</p>
      `
    };
...
export const resetPassword = async (req: Request, res: Response): Promise<Response> => {
  const { email, token, newPassword } = req.params;
