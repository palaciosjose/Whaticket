...
const AllConnections = () => {
  ...
  const loadCompanies = async () => {
    ...
    try {
      ...
      toast.error(i18n.t("No fue posible cargar la lista de registros"));
    }
...
<Typography variant="body2" align="center">
  {i18n.t("Conexiones conectadas")}:{" "}
  {whats?.length &&
    whats.filter((item) => item?.companyId === company?.id && item?.status === "CONNECTED").length}
</Typography>
<Typography variant="body2" align="center">
  {i18n.t("Conexiones desconectadas")}:{" "}
  {whats?.length &&
    whats.filter((item) => item?.companyId === company?.id && item?.status !== "CONNECTED").length}
</Typography>
<Typography variant="body2" align="center">
  {i18n.t("Total de Conexiones")}:{" "}
...
<Typography variant="body2" align="center">
  {i18n.t("Total de Conexiones")}: {whats?.length && whats.length}
</Typography>
        </CardContent>
      </Card>
    </Grid>
